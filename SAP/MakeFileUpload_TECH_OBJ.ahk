...
class MakeFileUpload_TECH_OBJ {

    static __New() {
        MakeFileUpload_TECH_OBJ.InitializeVariables()
        MakeFileUpload_TECH_OBJ.SetupEventListeners()
    }
    ...
    Static SetupEventListeners() {
        EventManager.Subscribe("MakeFile_TECH_OBJ", (data) => MakeFileUpload_TECH_OBJ.MakeFile_TECH_OBJ(data.flArray, data.flTechnology, data.flInvType))
    }

; ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
; * Realizzazione dei file per l'aggiornamento della Control Table Asset
;   Deve essere creato il file:
;   -   nomeFile_ZPMR_TECH_OBJ_UpLoad
;   
;   Da caricare attraverso la transazione: ZPM4R_UPL_FL_FILE
; ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

    ; Funzione per costruire file per aggioranmento della tabella TECH_OBJ
    ...
    ;~ Parametri:
    ;~ - un array contenente la lista degli elementi non presenti nella TECH_OBJ prodotta dalla verifica precedente
    ;~ Restituisce:
    ;~ - Due file pronti per il caricamento a sistema
    ;~ - False altrimenti

    static MakeFile_TECH_OBJ(arr, tech, invType:="") {
        MakeFileTECH_OBJ_result := { success: false, value: false, error: "", class: "MakeFileUpload_TECH_OBJ.ahk", function: "MakeFileTECH_OBJ" }
        EventManager.Publish("ProcessStarted", {processId: MakeFileTECH_OBJ_result.function, status: "Started", details: "Avvio funzione", result: {}})
        EventManager.Publish("PI_Start", {inputValue: "Crea file aggiornamento TECH_OBJ "}) ; Avvia l'indicatore di progresso 
            if (arr.Length = 0) {
                msgbox("Errore l'array indicato è vuoto!", "Errore", 4112)
                MakeFileTECH_OBJ_result.error := "Errore l'array indicato è vuoto!"
                EventManager.Publish("ProcessError", {processId: MakeFileTECH_OBJ_result.function, status: "Error", details: MakeFileTECH_OBJ_result.error, result: {}})
                return MakeFileTECH_OBJ_result  ; Se l'array è vuoto allora restituisco un errore
            }  
            ...
            MakeFileTECH_OBJ_result.success := true ; imposto a true e cambio in caso venga riscontrato almeno un errore
            ...
            try {
                EventManager.Publish("ProcessProgress", {processId: "MakeFile_TECH_OBJ", status: "In Progress", details: "Genero array contenente file guideline", result: {}})     
                if (tech = "S") { ; verifico gli impianti con tecnologia SOLAR
                    ...
                    switch invType
                    {
                        case 1:
                            ...
                            EventManager.Publish("DebugMsg",{msg: "MakeFile_TECH_OBJ: Selezionato Central Inverter", linenumber: A_LineNumber})
                        case 2:
                            arrFileGuideLine.Push(G_CONSTANTS.file_FL_Solar_StringInv)
                            EventManager.Publish("DebugMsg",{msg: "MakeFile_TECH_OBJ: Selezionato String Inverter", linenumber: A_LineNumber})
                        case 3:
                            arrFileGuideLine.Push(G_CONSTANTS.file_FL_Solar_InvModule)
                            EventManager.Publish("DebugMsg",{msg: "MakeFile_TECH_OBJ: Selezionato Inverter Module", linenumber: A_LineNumber})
                        default:
                            MakeFileTECH_OBJ_result.success := false
                            MakeFileTECH_OBJ_result.error := "Errore nella selezione tecnologia inverter"
                            ...
                            return MakeFileTECH_OBJ_result
                    }
                }
                ...
                EventManager.Publish("ProcessProgress", {processId: "MakeFile_TECH_OBJ", status: "In Progress", details: "Genero map a partire dalle guideline", result: {}})     
                map_Guideline := MakeFileUpload_TECH_OBJ.MakeMapGuideline(arrFileGuideLine)
                MakeString_TECH_OBJ_result := MakeFileUpload_TECH_OBJ.MakeString_TECH_OBJ(map_Guideline.value, arr) 
                ; genera un oggetto contenente il risutlato del metodo MakeString_TECH_OBJ_result
                ; result := { success: false, value: false, error: "", class: "MakeFileUpload_TECH_OBJ.ahk", function: "MakeString_TECH_OBJ" }
                nomeFile_ZPMR_TECH_OBJ := MakeFileUpload_TECH_OBJ.GetFileName(G_CONSTANTS.file_ZPMR_TECH_OBJ_UpLoad)
                EventManager.Publish("ProcessProgress", {processId: "MakeFile_TECH_OBJ", status: "In Progress", details: "Scrivo file " . nomeFile_ZPMR_TECH_OBJ, result: {}})     
                if (MakeString_TECH_OBJ_result.success = true) { ; se la creazione del file è andata a buon fine allora scrivo il file su disco
                    WriteStringToFile_result := MakeFileUpload_TECH_OBJ.WriteStringToFile(MakeString_TECH_OBJ_result.value, G_CONSTANTS.file_ZPMR_TECH_OBJ_UpLoad)
                    if (WriteStringToFile_result = false) {
                        errorMsg := "Errore nelle generazione del file TECH_OBJ" . " - LN: " . A_LineNumber
                        throw Error(errorMsg) 
                    }
                    else {
                        EventManager.Publish("AddLV", {icon: "icon4", element: nomeFile_ZPMR_TECH_OBJ, text: "File aggiornato"})
                        EventManager.Publish("ProcessCompleted", {processId: "MakeFile_TECH_OBJ", status: "Completed", details: "Esecuzione completata con successo", result: {}})
                        EventManager.Publish("PI_Stop", {inputValue: "File aggiornamento TECH_OBJ creato"}) ; Avvia l'indicatore di progresso
                        MakeFileTECH_OBJ_result.success := true
                        MakeFileTECH_OBJ_result.value := true
                        return MakeFileTECH_OBJ_result
                    }
                }
                else {
                ...
                MakeFileTECH_OBJ_result.error := "Errore: " . err.Message          
                EventManager.Publish("ProcessError", {processId: "MakeMapGuideline", status: "Error", details: MakeFileTECH_OBJ_result.error, result: {}})
                MsgBox(MakeFileTECH_OBJ_result.error, "Error", 4144)
                return MakeFileTECH_OBJ_result  
            }              
    }
    ...
    static MakeString_TECH_OBJ(mapGuideline, FL_Arr) {
        ...
        for element in FL_Arr {        
            ...
            for key, data in mapGuideline { ; considero tutti gli elementi della Guideline, la chiave è il pattern da verificare con espressione regolare
                if MakeFileUpload_TECH_OBJ.IsValidString(element, key) {
            ...
            Case 0: ; nessuna corrispondenza trovata
                ;~ MakeFileUpload_TECH_OBJ.mainGui.gui.LV.Add("icon3", element, "Errore Guideline")
                ;~ MakeFileUpload_TECH_OBJ.mainGui.gui.LV.ModifyCol(1, "autoHdr")
    ...
    static MakeMapGuideline(arrFilename) {
        result := { success: false, value: false, error: "", class: "MakeFileUpload_TECH_OBJ.ahk", function: "MakeMapGuideline" }
        ...
        if(arrFilename.length > 0) { ; verifico che l'array file contenga elementi
            ...
            for singleFile in arrFilename { ; per ogni file presente nell'array
                ...
                try {
                    ...
                    MapRules := MakeFileUpload_TECH_OBJ.MakeMapRules(G_CONSTANTS.file_Rules)
                        ...
                        if (line != "") {  ; Ignora le linee vuote
                            ...
                            if (LunghezzaFL >= 4) and (LunghezzaFL <= 6) {
                                ...
                                */    
                                ...
                                {
                                    case 4:
                                        patternKey := ("^" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[4], MapRules) . "_" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[3], MapRules) . "_4$")
                                    case 5:
                                        patternKey := ("^" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[5], MapRules) . "_" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[4], MapRules) . "_" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[3], MapRules) . "_5$")
                                    case 6:
                                        patternKey := ("^" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[6], MapRules) . "_" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[5], MapRules) . "_" . MakeFileUpload_TECH_OBJ.MakeSingeLevelPattern(patternFL[4], MapRules) . "_6$")
                                    default:
                                        continue
                                }
