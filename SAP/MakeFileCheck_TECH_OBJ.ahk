...
class MakeFileCheck_TECH_OBJ {

    static __New() {
        MakeFileCheck_TECH_OBJ.InitializeVariables()
        MakeFileCheck_TECH_OBJ.SetupEventListeners()
    }
    ...
    Static SetupEventListeners() {
        EventManager.Subscribe("MakeFileCheck_TECH_OBJ", (data) => MakeFileCheck_TECH_OBJ.Check_TECH_OBJ(data.flArray, data.flTechnology, data.flInvType))
    }

; ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
; * Crea un file contentente tutti i Tech Obj per la lista completa di TUTTE le FL presenti nella GUI
;   
; ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

    ; Funzione per generare un file contenente la lista completa delle Tech Obj a partire dalla lista completa della FL 
    ...
    ;~ Parametri:
    ;~ - un array contenente la lista completa delle FL
    ;~ Restituisce:
    ;~ - Genera un file contenente la lista completa delle Tech Obj
    ;~ - False altrimenti

    static Check_TECH_OBJ(arr, tech, invType:="") {
        MakeFileCheck_TECH_OBJ_result := { success: false, value: false, error: "", class: "MakeFileCheck_TECH_OBJ.ahk", function: "Check_TECH_OBJ" }
        EventManager.Publish("ProcessStarted", {processId: MakeFileCheck_TECH_OBJ_result.function, status: "Started", details: "Avvio funzione", result: {}})
        EventManager.Publish("PI_Start", {inputValue: "Crea file per la verifica delle TECH_OBJ nelle FL"}) ; Avvia l'indicatore di progresso 
            if (arr.Length = 0) {
                msgbox("Errore l'array indicato è vuoto!", "Errore", 4112)
                MakeFileCheck_TECH_OBJ_result.error := "Errore l'array indicato è vuoto!"
                EventManager.Publish("ProcessError", {processId: MakeFileCheck_TECH_OBJ_result.function, status: "Error", details: MakeFileCheck_TECH_OBJ_result.error, result: {}})
                return MakeFileCheck_TECH_OBJ_result  ; Se l'array è vuoto allora restituisco un errore
            }  
            ...
            MakeFileCheck_TECH_OBJ_result.success := true ; imposto a true e cambio in caso venga riscontrato almeno un errore
            ...
            ; {VALUE: "", SUB_VALUE: "", SUB_VALUE2: "", TPLKZ: "", FLTYP: "", FLLEVEL: "", EQART: "", RBNR: "", NATURE: "", DOCUMENTO_PM: ""}
            try {
                EventManager.Publish("ProcessProgress", {processId: MakeFileCheck_TECH_OBJ_result.function, status: "In Progress", details: "Genero array contenente file guideline", result: {}})     
                if (tech = "S") { ; verifico gli impianti con tecnologia SOLAR
                    ...
                    switch invType
                    {
                        case 1:
                            ...
                            EventManager.Publish("DebugMsg",{msg: "MakeFileCheck_TECH_OBJ: Selezionato Central Inverter", linenumber: A_LineNumber})
                        case 2:
                            arrFileGuideLine.Push(G_CONSTANTS.file_FL_Solar_StringInv)
                            EventManager.Publish("DebugMsg",{msg: "MakeFileCheck_TECH_OBJ: Selezionato String Inverter", linenumber: A_LineNumber})
                        case 3:
                            arrFileGuideLine.Push(G_CONSTANTS.file_FL_Solar_InvModule)
                            EventManager.Publish("DebugMsg",{msg: "MakeFileCheck_TECH_OBJ: Selezionato Inverter Module", linenumber: A_LineNumber})
                        default:
                            MakeFileCheck_TECH_OBJ_result.success := false
                            MakeFileCheck_TECH_OBJ_result.error := "Errore nella selezione tecnologia inverter"
                            ...
                            return MakeFileCheck_TECH_OBJ_result
                    }
                }
                ...
                EventManager.Publish("ProcessProgress", {processId: MakeFileCheck_TECH_OBJ_result.function, status: "In Progress", details: "Genero map a partire dalle guideline", result: {}})     
                map_Guideline := MakeFileCheck_TECH_OBJ.MakeMapGuideline(arrFileGuideLine)
                ; ricavo a partire dall' array contenente tutte le FL i valori necessari al controllo della funzione MakeString_TECH_OBJ
                FL_array_Conc := MakeFileCheck_TECH_OBJ.MakeFL_Conc(arr)
                FL_array_Conc := MakeFileCheck_TECH_OBJ.RemoveDuplicates(FL_array_Conc)
                MakeString_TECH_OBJ_result := MakeFileCheck_TECH_OBJ.MakeString_TECH_OBJ(map_Guideline.value, FL_array_Conc) 
                ; genera un oggetto contenente il risutlato del metodo MakeString_TECH_OBJ_result
                ; result := { success: false, value: false, error: "", class: "MakeFileCheck_TECH_OBJ.ahk", function: "MakeString_TECH_OBJ" }
                nomeFile_CHECK_TECH_OBJ := MakeFileCheck_TECH_OBJ.GetFileName(G_CONSTANTS.file_Check_TECH_OBJ)
                EventManager.Publish("ProcessProgress", {processId: MakeFileCheck_TECH_OBJ_result.function, status: "In Progress", details: "Scrivo file " . nomeFile_CHECK_TECH_OBJ, result: {}})     
                if (MakeString_TECH_OBJ_result.success = true) { ; se la creazione del file è andata a buon fine allora scrivo il file su disco
                    WriteStringToFile_result := MakeFileCheck_TECH_OBJ.WriteStringToFile(MakeString_TECH_OBJ_result.value, G_CONSTANTS.file_Check_TECH_OBJ)
                    if (WriteStringToFile_result = false) {
                        errorMsg := "Errore nelle generazione del file TECH_OBJ" . " - LN: " . A_LineNumber
                        throw Error(errorMsg) 
                    }
                    else {
                        EventManager.Publish("AddLV", {icon: "icon4", element: nomeFile_CHECK_TECH_OBJ, text: "File aggiornato"})
                        EventManager.Publish("ProcessCompleted", {processId: MakeFileCheck_TECH_OBJ_result.function, status: "Completed", details: "Esecuzione completata con successo", result: {}})
                        EventManager.Publish("PI_Stop", {inputValue: "File aggiornamento TECH_OBJ creato"}) ; Avvia l'indicatore di progresso
                        MakeFileCheck_TECH_OBJ_result.success := true
                        MakeFileCheck_TECH_OBJ_result.value := true
                        return MakeFileCheck_TECH_OBJ_result
                    }
                }
                else {
                    errorMsg := "Errore nelle generazione della stringa TECH_OBJ" . " - LN: " . A_LineNumber
                    throw Error(errorMsg) 
                }                
            }    
            catch as err {
                MakeFileCheck_TECH_OBJ_result.error := "Errore: " . err.Message          
                EventManager.Publish("ProcessError", {processId: "MakeMapGuideline", status: "Error", details: MakeFileCheck_TECH_OBJ_result.error, result: {}})
                MsgBox(MakeFileCheck_TECH_OBJ_result.error, "Error", 4144)
                return MakeFileCheck_TECH_OBJ_result  
            }              
    }

    ; Funzione: MakeString_TECH_OBJ
    ; Descrizione: esamina ogni elemento presente nell'array contenente la lista delle FL non presenti nella TECH_OBJ e lo confronta con il pattern
    ; ricavato dalla guideline tramite la funzione MakeMapGuideline.
    ; Crea una stringa da utilizzare per creare il file di caricamento per l'aggiornamento della TECH_OBJ
    ; Parametri:
    ;   - param1:   il map() contenente come chiave il pattern delle FL ricavato a partire dalle guideline 
    ;   - param2:   array contenente la lista delle FL non presenti nella tabella deolle TECH_OBJ
    ; Restituisce:  Una stringa  per generare il file di caricamento per l'aggiornamento della TECH_OBJ. 
    ;               Considera solo le FL con lunghezza pari a 4, 5, 6.
    ; Esempio: 
    ; 
    static MakeFL_Conc(flArray) {
        Lista_FL_Conc:=[]
        for element in flArray {
            ...
            if (numberOfElement >= 4) {
                ...
                {
                case 4: concatena := MakeFileCheck_TECH_OBJ.Concatena(temp_array, 2)
                case 5: concatena := MakeFileCheck_TECH_OBJ.Concatena(temp_array, 3)
                case 6: concatena := MakeFileCheck_TECH_OBJ.Concatena(temp_array, 3)
                }
                ; memorizzo i valori in un array
                Lista_FL_Conc.Push(concatena)
            }
        }
        ; se l'array non contiene elementi ritorno un valore falso
        if (Lista_FL_Conc.Length = 0) {
            return false
        }
        else { ; se non contiene elementi concludo 
            OutputDebug("Check Tech Obj: Restituisco la lista concatenata degli elementi della FL con lunghezza >= 4`n")
            return Lista_FL_Conc
        }
    }

    ; Funzione per rimuovere i valori duplicati da un array
    static RemoveDuplicates(inputArray) {
    ...
    ; Esempio: 
    ; 
    static MakeString_TECH_OBJ(mapGuideline, FL_Arr) {
        result := { success: false, value: false, error: "", class: "MakeFileCheck_TECH_OBJ.ahk", function: "MakeString_TECH_OBJ" }
        content_TECH_OBJ := ""
        rowCount := 0
        for element in FL_Arr {        
            ...
            for key, data in mapGuideline { ; considero tutti gli elementi della Guideline, la chiave è il pattern da verificare con espressione regolare
                if MakeFileCheck_TECH_OBJ.IsValidString(element, key) {
                    ...
                    key_value .= "`t`t`t" . key . "`n"
                    myData := data
                    ;data := {VALUE: "", SUB_VALUE: "", SUB_VALUE2: "", TPLKZ: "", FLTYP: "", FLLEVEL: "", EQART: "", RBNR: "", NATURE: "", DOCUMENTO_PM: ""}
            ...
            Case 0: ; nessuna corrispondenza trovata
                ;~ MakeFileCheck_TECH_OBJ.mainGui.gui.LV.Add("icon3", element, "Errore Guideline")
                ;~ MakeFileCheck_TECH_OBJ.mainGui.gui.LV.ModifyCol(1, "autoHdr")
                ;~ msgbox("Nessuna corrispondenza trovato nella guideline per: " . element)
                EventManager.Publish("AddLV", {icon: "icon3", element: element, text: "TECH_OBJ: Nessuna corrispondenza trovata in Guideline"})
                result.error := element . " - Nessuna corrispondenza trovata. `n"
            Case 1: ; una corrispondenza trovata
                ;EventManager.Publish("Debug",("MakeString_TECH_OBJ: l'elemento " . element . " è corretto."))
                content_TECH_OBJ .= myData.VALUE . ";" . myData.SUB_VALUE . ";" . myData.SUB_VALUE2 . ";" . myData.TPLKZ . ";" . myData.FLTYP . ";" . myData.FLLEVEL . ";" . myData.EQART . ";" . myData.RBNR . ";" . myData.NATURE . ";" . myData.DOCUMENTO_PM . "`r`n"
                rowCount++
            Default:
                OutputDebug("Molteplici elementi soddisfano criteri guideline: `n`tFL: `t" element . "`n`tGuideLine: `t" key_value)
/*              
                EventManager.Publish("AddLV", {icon: "icon2", element: element, text: "TECH_OBJ: Molteplici corrispondenze in Guideline"})
...
*/                
        ...
        OutputDebug("Elementi presenti in array: " . FL_Arr.length . " - Righe generate in TECH_OBJ: " . rowCount . "`n")
        if (result.error = "") {
            result.value := G_CONSTANTS.intestazione_TECH_OBJ . content_TECH_OBJ
            result.success := true
        }
        return result
    }
    ...
    ; Descrizione: Legge il contenuto di un file delle guideline conentuti nell'array  e crea un map() contenente gli elementi presenti nella tabella e necessari alla creazione dei file
    ; Parametri:
    ...
    static MakeMapGuideline(arrFilename) {
        result := { success: false, value: false, error: "", class: "MakeFileCheck_TECH_OBJ.ahk", function: "MakeMapGuideline" }
        ...
        if(arrFilename.length > 0) { ; verifico che l'array file contenga elementi
            ...
            for singleFile in arrFilename { ; per ogni file presente nell'array
                ...
                try {
                    ...
                    MapRules := MakeFileCheck_TECH_OBJ.MakeMapRules(G_CONSTANTS.file_Rules)
                    ...
                    for line in lines {
                        ...
                        if (line != "") {  ; Ignora le linee vuote
                            ...
                            if (LunghezzaFL >= 4) and (LunghezzaFL <= 6) {
                                ...
                                data.EQART := element[7] ; Tech.Obj.SAP CODE
                                data.RBNR := element[8] ; Catalog Profile
                                ...
                                */    
                                ...
                                {
                                    case 4:
                                        patternKey := ("^" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[4], MapRules) . "_" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[3], MapRules) . "_4$")
                                    case 5:
                                        patternKey := ("^" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[5], MapRules) . "_" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[4], MapRules) . "_" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[3], MapRules) . "_5$")
                                    case 6:
                                        patternKey := ("^" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[6], MapRules) . "_" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[5], MapRules) . "_" . MakeFileCheck_TECH_OBJ.MakeSingeLevelPattern(patternFL[4], MapRules) . "_6$")
                                    default:
                                        continue
                                }
